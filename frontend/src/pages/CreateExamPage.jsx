...
export default function CreateExamPage() {
  ...
  const addField = () => setCustomFields([...customFields, {
    key: `field_${Date.now()}`, label: '', type: 'text', showOnCertificate: true, required: false
  }]);
  ...
  const Label = ({ children, required }) => (
    <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: '#666', marginBottom: 5,
  ...
  const Card = ({ title, children }) => (
    <div style={{ background: '#fff', borderRadius: 12, padding: 28,
      ...
      <h2 style={{ fontSize: 15, fontWeight: 700, color: '#1a1a2e', margin: '0 0 20px' }}>{title}</h2>
      {children}
    </div>
  );
  ...
  return (
    <div style={{ fontFamily: "'Inter', -apple-system, sans-serif", maxWidth: 740 }}>
      <div style={{ marginBottom: 28 }}>
        <h1 style={{ fontSize: 24, fontWeight: 700, color: '#1a1a2e', margin: 0 }}>Create New Exam</h1>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <Card title="📝 Exam Details">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            <div style={{ gridColumn: '1 / -1' }}>
              ...
              <input required style={s} value={form.title} onChange={set('title')} placeholder="e.g., Web Development Fundamentals" />
            </div>
            <div style={{ gridColumn: '1 / -1' }}>
              <Label>Description</Label>
              <textarea style={{ ...s, minHeight: 72, resize: 'vertical' }} value={form.description} onChange={set('description')} placeholder="Brief description..." />
            </div>
            <div>
              <Label required>Instructor Name</Label>
              <input required style={s} value={form.instructor} onChange={set('instructor')} placeholder="Dr. Jane Smith" />
            </div>
            <div>
              <Label>Organization</Label>
              <input style={s} value={form.organization} onChange={set('organization')} placeholder="Tech Academy" />
            </div>
            <div>
              <Label required>Passing Score (%)</Label>
              <input required type="number" min="0" max="100" style={s} value={form.passingScore} onChange={set('passingScore')} />
            </div>
            <div>
              <Label>Duration</Label>
              <input style={s} value={form.duration} onChange={set('duration')} placeholder="e.g., 3 hours" />
            </div>
          </div>
        </Card>
        ...
        <Card title="🎨 Certificate Design">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 20 }}>
            {TEMPLATES.map(t => (
              <div key={t.id} onClick={() => setForm({ ...form, certificateTemplate: t.id })} style={{
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            ...
            <input style={{ ...s, flex: 1 }} value={form.primaryColor} onChange={set('primaryColor')} placeholder="#1a73e8" />
          </div>
        </Card>
        ...
        <Card title="⚙️ Custom Certificate Fields">
          <p style={{ fontSize: 13, color: '#888', margin: '0 0 16px', lineHeight: 1.5 }}>
            Add extra fields that will appear on each participant's form and optionally print on the certificate.
            Examples: <em>Batch / Cohort, Venue, Department, Student ID</em>.
          </p>

          {customFields.map((field, idx) => (
            <div key={idx} style={{
              border: '1.5px solid #e8e8e8', borderRadius: 10, padding: 16, marginBottom: 12,
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: 12, alignItems: 'end' }}>
                <div>
                  ...
                  <input
                    style={s} value={field.label} placeholder="e.g., Batch / Cohort"
                    onChange={e => updateField(idx, { label: e.target.value })}
                  />
                </div>
                <div>
                  <Label>Data Type</Label>
                  <select style={s} value={field.type} onChange={e => updateField(idx, { type: e.target.value })}>
                    {FIELD_TYPE_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
                  </select>
                </div>
                <button type="button" onClick={() => removeField(idx)} style={{
              ...
              <div style={{ display: 'flex', gap: 20, marginTop: 12 }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: 13, color: '#444' }}>
                  <input type="checkbox" checked={field.showOnCertificate}
              ...
              <div style={{ fontSize: 11, color: '#aaa', marginTop: 8 }}>Key: <code>{field.key}</code></div>
            </div>
          ))}
