...
export default function CreateExamPage() {
  ...
  const [form, setForm] = useState({
    ...
    duration: '', certificateTemplate: 'modern', primaryColor: '#1a73e8',
  });
      ...
      <form onSubmit={handleSubmit}>
        ...
        <Card title="📝 Exam Details">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            <div style={{ gridColumn: '1 / -1' }}>
              ...
              <input required style={s} value={form.title} onChange={set('title')}
                placeholder="e.g., Annual National Mathematics Competition" />
            </div>
            <div style={{ gridColumn: '1 / -1' }}>
              <Label>Description</Label>
              <textarea style={{ ...s, minHeight: 72, resize: 'vertical' }}
                value={form.description} onChange={set('description')}
                placeholder="Brief description..." />
            </div>
            <div>
              <Label required>Instructor Name</Label>
              <input required style={s} value={form.instructor} onChange={set('instructor')}
                placeholder="Dr. Jane Smith" />
            </div>
            <div>
              <Label>Organization</Label>
              <input style={s} value={form.organization} onChange={set('organization')}
                placeholder="Nigerian Tulip International Colleges" />
            </div>
            <div>
              <Label>Duration</Label>
              <input style={s} value={form.duration} onChange={set('duration')}
        ...
        <Card title="🎨 Certificate Design">
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            ...
            <input style={{ ...s, flex: 1 }} value={form.primaryColor} onChange={set('primaryColor')}
              placeholder="#1a73e8" />
          </div>
        </Card>
        ...
        <Card title="⚙️ Custom Certificate Fields">
          <p style={{ fontSize: 13, color: '#888', margin: '0 0 16px', lineHeight: 1.5 }}>
            Add extra fields that appear on each participant's form and optionally print on the certificate.
            ...
            <div key={field.key} style={{
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: 12, alignItems: 'end' }}>
                <div>
                  ...
                  <input style={s} value={field.label} placeholder="e.g., Batch / Cohort"
                    onChange={e => updateField(idx, { label: e.target.value })} />
                </div>
                <div>
                  <Label>Data Type</Label>
                  <select style={s} value={field.type}
